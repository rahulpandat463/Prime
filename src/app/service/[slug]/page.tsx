...
async function getProfilesByService(serviceSlug: string) {
  const [rows] = await pool.query(`
...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  return {
    title: `${service.name} Services | Verified Listings`,
    description: `Find profiles offering ${service.name.toLowerCase()} services. 100% verified profiles.`,
  };
}
...
export default async function ServicePage({ params }: { params: { slug: string } }) {
  ...
  const profiles = await getProfilesByService(service.slug);
    ...
    <div className="min-h-screen bg-gray-50">
      <section className="bg-gradient-to-r from-pink-500 to-purple-600 text-white py-20">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-xl max-w-2xl mx-auto">
            Browse profiles offering {service.name.toLowerCase()} services with real photos and reviews.
          </p>
        </div>
      </section>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 gap-6 max-w-5xl mx-auto">
                  ...
                  <div>
                    <div className="flex justify-between items-start">
                      <h3 className="text-xl font-bold text-gray-800 leading-tight mb-2 group-hover:text-pink-600 transition line-clamp-2">
                        High Quality {profile.name} Service {service.name} Only Call And Whatsapp Me
                      </h3>
          ...
          <p className="text-gray-600">No profiles found offering this service.</p>
        )}
      </div>
    </div>
  );
