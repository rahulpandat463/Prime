...
export async function GET(request: NextRequest) {
  try {
    ...
    let query = 'SELECT id, name, slug, description, status, sort_order FROM services';
    const params: any[] = [];
    ...
    if (id) {
      ...
      query += ' WHERE status = "active" ORDER BY sort_order ASC, name ASC';
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const [result] = await pool.query(
      'INSERT INTO services (name, slug, description, status, sort_order) VALUES (?, ?, ?, ?, ?)',
